...
const ProfileMainScreen = ({ navigation }: ProfileMainScreenProps) => {
    ...
    try {
      ...
      if (storedUsername) {
        try {
          ...
          const response = await axios.get(`${API_BASE_URL}/users/${storedUsername}`);
  ...
  const uploadImage = async (imageAsset: any) => {
    ...
    try {
      ...
      console.log('Server URL:', `${API_BASE_URL}/users/${username}/upload-profile-image`);

      // Make POST request to upload the image
      const response = await axios.post(
        `${API_BASE_URL}/users/${username}/upload-profile-image`,
        formData,
        {
          headers: {
