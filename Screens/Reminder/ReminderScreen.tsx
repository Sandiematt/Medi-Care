...
const ReminderMainScreen: React.FC<ReminderMainScreenProps> = ({ navigation }) => {
  ...
  const fetchInventoryData = useCallback(async () => {
    try {
      ...
      const inventoryResponse = await fetch(`${API_BASE_URL}/inventory?username=${username}`);
  ...
  const fetchReminders = useCallback(async () => {
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/reminders/${username}`);
  ...
  const handleDeleteReminder = useCallback(async () => {
    ...
    try {
      const username = await AsyncStorage.getItem('username');
      const response = await fetch(`${API_BASE_URL}/reminders/${reminderToDelete._id}?username=${username}`, {
        method: 'DELETE',
      });
