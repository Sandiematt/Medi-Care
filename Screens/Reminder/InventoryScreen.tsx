...
const InventoryScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchInventoryItems = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/inventory?username=${encodeURIComponent(parsedUsername)}`);
  ...
  const fetchStats = async () => {
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/stats?username=${encodeURIComponent(parsedUsername)}`);
      if (!response.ok) {
  ...
  const handleUpdateItem = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/inventory/${editingItem._id}`, {
        method: 'PUT',
        headers: {
  ...
  const handleDeleteItem = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/inventory/${editingItem._id}?username=${encodeURIComponent(parsedUsername)}`, {
        method: 'DELETE',
      });
