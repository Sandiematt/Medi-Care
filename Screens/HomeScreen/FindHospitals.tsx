  ...
  const fetchNearbyHospitals = useCallback(async (latitude: number, longitude: number) => {
    try {
      ...
      const response = await fetch(
        `https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=${latitude},${longitude}&radius=15000&type=hospital&key=${process.env.GOOGLE_MAPS_API_KEY}`
      );
        ...
        const hospitalsData = await Promise.all(
          data.results.map(async (place: any) => {
            ...
            const detailsResponse = await fetch(
              `https://maps.googleapis.com/maps/api/place/details/json?place_id=${place.place_id}&fields=formatted_phone_number,website,opening_hours,photos&key=${process.env.GOOGLE_MAPS_API_KEY}`
            );
            ...
            if (place.photos && place.photos.length > 0) {
              photoUrl = `https://maps.googleapis.com/maps/api/place/photo?maxwidth=400&photoreference=${place.photos[0].photo_reference}&key=${process.env.GOOGLE_MAPS_API_KEY}`;
            }
